...
export function Paragraph({ node, children, ...props }: ComponentProps<"p"> & ExtraProps) {
  ...
  useEffect(() => {
    ...
    if (!element || streaming || !text || text.length < 512 || text.length > 8192) return
    let disposed = false
    let observer: ResizeObserver | undefined
    void Promise.all([import("@/lib/paragraph-geometry"), document.fonts.ready]).then(([geometry]) => {
      if (disposed) return
