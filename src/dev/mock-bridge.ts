...
export function installMockBridge() {
  ...
  window.pi = {
    ...
    threads: async () => ({ ready: true, threads: [
      ...
      { harness: "devin", nativeId: "dv-1", path: "/mock/devin.jsonl", cwd: "/Users/you/api", title: "Wire the payments retry queue", model: "adaptive", modelProvider: "devin", updatedAt: new Date(Date.now() - 2_400_000).toISOString(), startedAt: new Date(Date.now() - 6_000_000).toISOString(), bytes: 14_000 },
    ...
    accounts: async () => [
      { harness: "claude" as const, name: "default", email: "iamkashyab@gmail.com", dir: "~/.claude", active: true },
      { harness: "claude" as const, name: "kashyab@getverbiflow.com", email: "kashyab@getverbiflow.com", dir: "~/.subrouter/codex/claude/_p1", active: false, source: "subrouter" as const },
      { harness: "codex" as const, name: "default", email: "ambaranikashyab@gmail.com", dir: "~/.codex", active: false },
      { harness: "codex" as const, name: "personal", email: "personal@work.dev", dir: "~/.mako/accounts/codex/personal", active: true },
    ],
    ...
    harnessProfiles: async () => [
      { id: "claude", label: "Claude Code", available: true, transport: "acp" as const, defaultModel: "opus[1m]", capabilities: ["stream", "fork"], models: [{ id: "opus[1m]", label: "Opus 5", contextWindow: 1_000_000, options: [{ kind: "select" as const, id: "effort", label: "Reasoning", current: "high", values: ["low", "medium", "high", "xhigh", "max"].map((value) => ({ value, label: value })) }, { kind: "boolean" as const, id: "fast", label: "Fast mode", current: false }] }] },
      { id: "codex", label: "Codex", available: true, transport: "app-server" as const, defaultModel: "gpt-5.6-sol", capabilities: ["stream", "fork-at-turn"], models: [{ id: "gpt-5.6-sol", label: "GPT-5.6 Sol", options: [{ kind: "select" as const, id: "effort", label: "Reasoning", current: "medium", values: ["low", "medium", "high", "xhigh", "max", "ultra"].map((value) => ({ value, label: value })) }] }] },
      { id: "cursor", label: "Cursor", available: true, transport: "acp" as const, defaultModel: "claude-fable-5", capabilities: ["stream"], models: [{ id: "claude-fable-5", label: "Claude Fable 5", options: [] }] },
      { id: "grok", label: "Grok", available: false, transport: "acp" as const, capabilities: [], models: [] },
      { id: "devin", label: "Devin", available: true, transport: "acp" as const, defaultModel: "adaptive", capabilities: ["stream"], models: [{ id: "adaptive", label: "Adaptive", options: [] }] },
    ],
    harnessAvailability: async () => ({ codex: true, claude: true, cursor: true, grok: false, devin: true }),
    ...
    acpStart: async (harness: string, cwd: string) => ({ id: "acp-1", harness, cwd, status: "ready" as const, modes: [{ id: "default", name: "Always Ask" }, { id: "acceptEdits", name: "Accept Edits" }], currentMode: "default", configOptions: [] }),
    acpPrompt: async () => {},
    acpPermission: async () => {},
    acpSetMode: async () => {},
    acpCancel: async () => {},
    ...
    harnessTuning: async (harness: string) => ({
      ...
      label: harness,
      available: true,
      transport: harness === "codex" ? ("app-server" as const) : ("acp" as const),
      defaultModel: harness === "claude" ? "opus[1m]" : harness === "devin" ? "adaptive" : "gpt-5.6-sol",
      capabilities: ["stream"],
      models: [{
        id: harness === "claude" ? "opus[1m]" : harness === "devin" ? "adaptive" : "gpt-5.6-sol",
        label: harness === "claude" ? "Opus 5" : harness === "devin" ? "Adaptive" : "GPT-5.6 Sol",
        options: [{ kind: "select" as const, id: "effort", label: "Reasoning", current: "high", values: ["low", "medium", "high", "xhigh"].map((value) => ({ value, label: value })) }],
      }],
    }),
