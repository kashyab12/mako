...
export function installMockBridge() {
  ...
  window.pi = {
    ...
    threads: async () => ({ ready: true, threads: [
      ...
      { harness: "pi", nativeId: "pi-dv-1", path: "/mock/pi-devin.jsonl", cwd: "/Users/you/api", title: "Wire the payments retry queue", model: "devin-2", modelProvider: "devin", updatedAt: new Date(Date.now() - 2_400_000).toISOString(), startedAt: new Date(Date.now() - 6_000_000).toISOString(), bytes: 14_000 },
    ...
    harnessTuning: async (harness: string) =>
      harness === "cursor"
        ...
        : { models: harness === "claude" ? ["claude-fable-5[1m]", "claude-fable-5", "opus", "sonnet"] : ["gpt-5.6-sol", "gpt-5.3-codex"], efforts: ["low", "medium", "high", "xhigh"], fast: false, defaultModel: harness === "claude" ? "claude-fable-5[1m]" : "gpt-5.6-sol", defaultEffort: "high" },
