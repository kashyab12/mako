...
const toggleSchema = z.object({ kind: z.literal("set-enabled"), enabled: z.boolean() })
...
async function connect(): Promise<void> {
  ...
  connecting = true
  try {
    const settings = settingsSchema.parse(await chrome.storage.local.get(["enabled", "profileId"]))
    ...
    router = activeRouter
    next.onMessage.addListener((value: unknown) => {
      ...
      else if (message.kind === "request") void activeRouter.request(message.client, message.command)
    ...
    next.onDisconnect.addListener(() => {
      ...
      void status(failed ? "Open Mako and finish browser setup, then reconnect." : "Disconnected from Mako")
      chrome.alarms.create("reconnect", { delayInMinutes: 1 })
    })
    const browser = navigator.userAgent.includes("Edg/") ? "edge" : "chrome"
    next.postMessage({ kind: "hello", profileId, browser, label: `${browser === "edge" ? "Edge" : "Chrome"} profile ${profileId.slice(0, 6)}` })
  } catch {
...
chrome.debugger.onEvent.addListener((source, method, params) => router?.event(source, method, params))
