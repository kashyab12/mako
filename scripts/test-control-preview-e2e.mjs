...
if (!process.versions.electron) {
  ...
  const child = spawn(resolve("node_modules/.bin/electron"), [fileURLToPath(import.meta.url)], { stdio: "inherit", env: { ...process.env, MAKO_PREVIEW_TEST_ROOT: root } })
...
async function run() {
  ...
  try {
    ...
    const url = new URL("/scripts/control-preview-browser.html", process.env.MAKO_TEST_ORIGIN ?? "http://127.0.0.1:5174/")
    ...
    while (Date.now() < deadline) {
      outcome = await viewer.webContents.executeJavaScript("({status:document.getElementById('result')?.dataset.status,text:document.getElementById('result')?.textContent})")
    ...
    await writeFile(join(root, "result.json"), JSON.stringify({ ...outcome, preservedFocus: true, windows: 2 }, null, 2))
    console.log(`PASS chat-scoped browser/native preview. Evidence: ${root}`)
  } finally {
    viewer.destroy(); fixture.destroy(); app.quit()
  }
}
