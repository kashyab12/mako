...
const { providerStore, providerProfileKey } = await import("../src/state/providers")
...
const calls: { provider: string; options: LiveStartOptions; dispatched: LiveStartOptions }[] = []
const accepted: LiveStartOptions[] = []
const host = await startWebHost(socket, async (channel, args) => {
  ...
  const snapshot: LiveSnapshot = {
    session: { id: options.conversationId, harness: provider, cwd, status: "starting", connection: "starting", modes: [], currentMode: null, configOptions: [] },
    revision: 1, createdAt: 1, blocks: [], base: null, permissions: [],
    requests: options.initialRequest ? [{ ...options.initialRequest, status: "queued" }] : [],
  }
...
const start = mock.method(bridge, "liveStart", async (...[provider, cwd, options]: Parameters<typeof bridge.liveStart>) => {
  const args = hostCallInputs["mako:live-start"].parse([provider, cwd, options])
  const envelope = { channel: "mako:live-start", args: args.map((value) => ({ kind: "value", value })) }
  assert.equal(RuntimeCallSchema.safeParse(envelope).success, false, "Actual launch objects reproduce the old pre-serialization failure")
  const browserWire = RuntimeCallSchema.parse(JSON.parse(JSON.stringify(envelope)))
  ...
  assert.deepEqual(browserWire.args[2], { kind: "value", value: dispatched }, "Desktop and browser serialization must agree")
...
try {
  for (const driver of providerHost.liveDrivers.list()) {
    ...
    const reset = () => {
      ...
      threadsStore.set({ viewing: null, opening: null, composerHarness: provider })
      prefsStore.set({ providerModes: {}, providerSettings: {}, settingsOverrides: {} })
      providerStore.set({ contexts: { [providerProfileKey(provider, root)]: { id: provider, label: profile.label, available: true, transport: profile.transport, capabilities: [], models: [] } } })
    }
    reset()
    ...
    reset()
    const empty = beginStart({ harness: provider, cwd: root, settingsTarget: { kind: "new", harness: provider, cwd: root }, blocks: [], hiddenUserPrompt: null })
    ...
    const configured = beginStart({ harness: provider, cwd: root, blocks: [{ type: "user", text: "Configured prompt" }], hiddenUserPrompt: null })
    const tuning = { model: "fixture-model", options: { fast: false, effort: "high" } }
    assert.equal(await launch(configured, { tuning }, "Configured prompt", [{ name: "file.txt", mimeType: "text/plain", size: 0, path: "/retained/file.txt", data: undefined }]), true)
    call = last(provider)
    assert.equal(call.dispatched.modeId, "saved-fixture-mode")
    assert.deepEqual(call.dispatched.tuning, tuning)
    assert.equal(Object.hasOwn(call.dispatched.initialRequest!.attachments[0]!, "data"), false)
    reset()
    prefsStore.set({ providerModes: { [provider]: "saved-fixture-mode" } })
    const explicit = beginStart({ harness: provider, cwd: root, blocks: [], hiddenUserPrompt: null })
    assert.equal(await launch(explicit, { modeId: "explicit-fixture-mode", tuning }), true)
    assert.equal(last(provider).dispatched.modeId, "explicit-fixture-mode")
    reset()
    assert.equal(await acp.startFresh(provider, root, "Read retained context, then continue", [], "Continue visibly"), true)
    call = last(provider)
    assert.equal(call.dispatched.displayPrompt, "Continue visibly")
    assert.equal(call.dispatched.initialRequest?.text, "Read retained context, then continue")
    if (driver.canResume) {
      reset()
      const ref = { harness: provider, nativeId: "fixture-native-id", path: join(root, "native-session"), cwd: root, title: "Saved thread" }
    ...
    rows.push({ provider, transport: profile.transport, freshDefaults: "passed", noInitialPrompt: "passed", nestedSettingsAndAttachment: "passed", savedAndExplicitModeEncoding: "passed", displayPrompt: "passed", resumeEncoding: driver.canResume ? "passed" : "not advertised" })
  }
  assert.ok(rows.some((row) => row.provider === "devin"))
  console.log(JSON.stringify({ kind: "Production launch actions and both wire representations; provider execution is stubbed, not a CLI/authentication test", rows }, null, 2))
} finally {
  start.mock.restore()
  host.close()
