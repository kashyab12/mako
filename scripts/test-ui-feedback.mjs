...
try {
  ...
  assert.equal(
    contexts.length,
    0,
    "zero volume must not allocate audio resources"
  )
  ...
  assert.equal(
    context.voices[0].disconnects,
    1,
    "a replacement disconnects the previous voice"
  )
  assert.equal(
    context.gains[1].disconnects,
    1,
    "a replacement also disconnects its envelope"
  )
  ...
  assert.equal(
    context.voices.length,
    2,
    "mute during decode must not create a voice later"
  )
  ...
  assert.equal(
    ...
    0.4,
    "cached samples use current volume"
  )
  assert.equal(fetches, 1)
  context.voices[2].onended()
  assert.equal(
    context.voices[2].disconnects,
    1,
    "finished samples release their node"
  )
  ...
  assert.equal(
    context.voices.length,
    3,
    "a disposed resume must not create a stale voice"
  )
  ...
  await flush()
  assert.equal(
    newContext.decodes.length,
    1,
    "an obsolete rejection cannot clear the new decode cache"
  )
  newContext.decodes[0].resolve({ duration: 0.1 })
  await Promise.all([fresh, overlapping])
  assert.equal(
    newContext.voices.length,
    1,
    "only the latest request survives async work"
  )
  retry.dispose()
  console.log(
    "UI feedback: mute, overlap, decode sharing, cleanup, current volume, and disposal races passed"
  )
} finally {
  for (const [key, descriptor] of [
    ["AudioContext", original.context],
