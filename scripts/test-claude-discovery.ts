...
`, { mode: 0o700 })
const env = { ...process.env, CLAUDE_CODE_EXECUTABLE: executable, CLAUDE_CONFIG_DIR: root }
const row = z.object({ request: z.enum(["list_models", "get_settings"]), strict: z.boolean() })
const requests = async () => (await readFile(calls, "utf8")).trim().split("\n").filter(Boolean).map((line) => row.parse(JSON.parse(line)))
const full = claudeProfileLoader.load(env, root)
void full.catch(() => {})
try {
  const deadline = Date.now() + 5000
  while (!(await requests()).some((entry) => entry.request === "get_settings")) {
  ...
  const quick = await Promise.race([
    ...
    delay(1000).then(() => { throw new Error("Launch waited for unrelated model defaults") }),
  ])
  assert.equal(quick.models.length, 2)
  assert.equal(quick.models[0].options.find((option) => option.id === "effort")?.current, undefined)
  assert.deepEqual(resolveHarnessTuning(quick, { model: "fixture-model", options: { effort: "low" } }), { model: "fixture-model", options: { effort: "low" } })
  assert.throws(() => resolveHarnessTuning(quick, { model: "fixture-model", options: { effort: "unsupported" } }), /not supported/)
  assert.equal((await requests()).filter((entry) => entry.request === "list_models").length, 1)
  await writeFile(gate, "ready")
  const detailed = await full
  assert.equal(detailed.models[0].options.find((option) => option.id === "effort")?.current, "high")
  assert.equal(quick.models[0].options.find((option) => option.id === "effort")?.current, undefined, "Background enrichment cannot mutate a launch catalogue")
  assert.ok((await requests()).every((entry) => entry.strict), "Read-only discovery must not start MCP servers")
  assert.ok(!JSON.stringify(detailed).includes("private-setting-fixture"))
  console.log("Claude discovery: launch validates the catalogue without waiting for defaults, background defaults remain exact, MCP startup is excluded, and private settings stay redacted")
} finally {
  await writeFile(gate, "ready")
  await full.catch(() => {})
